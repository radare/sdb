...
SDB_API ut64 sdb_num_inc(Sdb *s, const char *key, ut64 n2, SdbCas *cas) {
	SdbCas c;
...
SDB_API ut64 sdb_num_dec(Sdb *s, const char *key, ut64 n2, SdbCas *cas) {
	SdbCas c;
	ut64 n = sdb_num_get (s, key, &c);
	if (cas && c != *cas) {
		return 0LL;
...
SDB_API int sdb_num_min(Sdb *db, const char*k, ut64 n, SdbCas *cas) {
	const char* a = sdb_const_get (db, k, NULL);
	return (!a || n < sdb_atoi (a))
		? sdb_num_set (db, k, n, cas): 0;
}

SDB_API int sdb_num_max(Sdb *db, const char*k, ut64 n, SdbCas *cas) {
	const char* a = sdb_const_get (db, k, NULL);
	return (!a || n > sdb_atoi (a))
		? sdb_num_set (db, k, n, cas): 0;
}
